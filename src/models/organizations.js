...
const getAllOrganizations = async () => {
    const query = 'SELECT organization_id, name, description, contact_email, logo_filename FROM public.organizations';
 ...
 * @param {string} logoFilename - the filename of the organization logo
...
const createOrganization = async (name, description, contactEmail, logoFilename) => {
    ...
    if (process.env.ENABLE_SQL_LOGGING === 'true') {
        console.log('Created new organizaion with ID:', result.rows[0].organization_id);         
    }
...
const updateOrganization = async (organizationId, name, description, contactEmail, logoFilename) => {
    ...
    if (process.env.ENABLE_SQL_LOGGING === 'true') {
        console.log('Organizaion with ID was updated correctly:', result.rows[0].organization_id);         
    }
